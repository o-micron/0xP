...
template<typename T>
void
processTextureLoading(XPSWScene<T>&     scene,
                      const aiScene*    aiScene,
                      const aiMaterial* aiMaterial,
                      aiTextureType     type,
                      bool&             hasTexture,
                      XPSWTexture2D&    texture)
{
    aiString       texPath;
    ...
    if (ret == AI_SUCCESS) {
        ...
        if (embeddedTexture) {
            ...
            if (embeddedTexture->mHeight == 0) {
                ...
                XP_SW_ASSERT_ERROR(texture.width * texture.height == embeddedTexture->mWidth,
            ...
            if (hint.substr(0, 4) == "rgba") {
                ...
                if (hint.substr(4, 4) == "8888") {
                    ...
                    XP_SW_ASSERT_ERROR(false, "We do not handle such cases yet.");
                }
...
template<typename T>
void
processMaterial(XPSWScene<T>& scene, const aiScene* aiScene, aiMaterial* aiMaterial, XPSWMaterial<T>& material)
{
    ...
    }
    {
        ...
        if (material.hasEmissionColorTexture == false) { material.emissionColorValue = XPVec3<T>{ 0.0f, 0.0f, 0.0f }; }
    }
    {
            ...
            std::unordered_set<std::string>&        uniqueLightNames)
{
    ...
    for (unsigned int i = 0; i < node->mNumMeshes; i++) {
        ...
        XPSWMesh<T>& meshData    = scene.meshes.back();
        meshData.name            = aiMesh->mName.C_Str();
        meshData.transform       = XPMat4<T>::identity();
        meshData.boundingBox.min = XPVec3<T>{ std::numeric_limits<T>::infinity(),
                                              std::numeric_limits<T>::infinity(),
                                              std::numeric_limits<T>::infinity() };
        meshData.boundingBox.max = XPVec3<T>{ -std::numeric_limits<T>::infinity(),
                                              -std::numeric_limits<T>::infinity(),
                                              -std::numeric_limits<T>::infinity() };
        ...
        // process materials
        {
            ...
            XPSWMaterial<T>& material   = scene.materials[aiMesh->mMaterialIndex];
        ...
        for (unsigned int j = 0; j < aiMesh->mNumVertices; j++) {
            ...
            meshData.colors.push_back(XPVec3<T>{ 1.0f, 1.0f, 1.0f });
        }

        // Extract normals and apply the normal matrix
        XP_SW_ASSERT_ERROR(aiMesh->HasNormals(), "Expecting a mesh with normals");
        ...
        XP_SW_ASSERT_ERROR(aiMesh->HasTextureCoords(0), "Expecting a mesh with texture coordinates");
    ...
    for (unsigned int i = 0; i < aiScene->mNumCameras; i++) {
        ...
        if (cameraNode) {
            ...
            XPSWCamera<T>& camera = scene.cameras.back();
            camera.name           = aiCam->mName.C_Str();
            camera.fov            = glm::degrees(aiCam->mHorizontalFOV / 2.0f);
            camera.resolution     = XPVec2<uint32_t>{ WIDTH, HEIGHT };
            ...
            camera.target.glm = camera.location.glm + camera.target.glm;
    ...
    for (unsigned int i = 0; i < aiScene->mNumLights; i++) {
        ...
        if (lightNode) {
            ...
            XPSWLight<T>& light = scene.lights.back();
            light.name          = aiLight->mName.C_Str();
            light.location      = XPVec3<T>{ lightPosition.x, lightPosition.y, lightPosition.z };
            light.direction     = XPVec3<T>{ lightDirection.x, lightDirection.y, lightDirection.z };
            light.ambient       = XPVec3<T>{ static_cast<T>(aiLight->mColorAmbient.r),
                                             static_cast<T>(aiLight->mColorAmbient.g),
                                             static_cast<T>(aiLight->mColorAmbient.b) };
            light.diffuse  = XPVec3<T>{ aiLight->mColorDiffuse.r, aiLight->mColorDiffuse.g, aiLight->mColorDiffuse.b };
            light.specular = XPVec3<T>{ static_cast<T>(aiLight->mColorSpecular.r),
            ...
            light.type                 = aiLight->mType == aiLightSource_DIRECTIONAL
                                           ? XPSWELightType_Directional
                                           : (aiLight->mType == aiLightSource_POINT ? XPSWELightType_Point : XPSWELightType_Spot);
            light.angleInnerCone       = aiLight->mAngleInnerCone;
            light.angleOuterCone       = aiLight->mAngleOuterCone;

            if (light.type == XPSWELightType_Directional &&
                (light.direction.x == 0 && light.direction.y == 0 && light.direction.z == 0)) {
                light.direction = XPVec3<T>{ 0.0f, -1.0f, 0.0f };
            }
...
importAsset(XPSWScene<T>& scene)
{
    ...
    if (scene.cameras.size() == 0) {
        ...
        camera.location       = XPVec3<T>{ static_cast<T>(0), static_cast<T>(0), static_cast<T>(0) };
        camera.target         = XPVec3<T>{ static_cast<T>(0), static_cast<T>(0), static_cast<T>(1) };
        camera.up             = XPVec3<T>{ static_cast<T>(0), static_cast<T>(1), static_cast<T>(0) };
        camera.updateMatrices();
    }
    ...
    if (scene.lights.size() == 0) {
        ...
        light.direction            = XPVec3<T>{ static_cast<T>(0), static_cast<T>(0), static_cast<T>(-1) };
        light.ambient              = XPVec3<T>{ static_cast<T>(1.0), static_cast<T>(1.0), static_cast<T>(1.0) };
        light.diffuse              = XPVec3<T>{ static_cast<T>(100.0), static_cast<T>(100.0), static_cast<T>(100.0) };
        light.specular             = XPVec3<T>{ static_cast<T>(100.0), static_cast<T>(100.0), static_cast<T>(100.0) };
        ...
        light.type                 = XPSWELightType_Point;
...
template<typename T>
bool
importDownloadedMeshAsset(XPSWScene<T>& scene, const void* pBuffer, size_t pLength)
{
