...
BoundingBox<T>::testFrustum(const std::array<Plane<T>, 6>& frustum) const
{
    ...
    for (int i = 0; i < 6; i++) {
        ...
        if (plane.distanceFromPoint(negativeCorner) < 0) { isFullyInside = false; }
    }
...
FramebufferRef::FramebufferRef(float* preAllocatedColorAttachmentPtr,
                               float* preAllocatedDepthAttachmentPtr,
                               int    width,
                               int    height)
