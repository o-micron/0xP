...
float
func(vec2 q, out vec4 ron, float time)
{
...
vec4
WarpingProcedural2(vec2 fragCoord, float time, vec2 resolution)
{
    vec2  p = fragCoord * 2.0 - 1.0;
    float e = 2.0 / resolution.y;

    vec4  on = vec4(0.0);
    float f  = func(p, on, time);
    ...
    vec4 kk;
    vec3 nor = normalize(vec3(func(p + vec2(e, 0.0), kk, time) - f, 2.0 * e, func(p + vec2(0.0, e), kk, time) - f));
...
main()
{
    FragColorChannel0 = WarpingProcedural2(vec2(1,1), 1, vec2(1,1));
}