...
vec2
sdStick(vec3 p, vec3 a, vec3 b, float r1, float r2) // approximated
{
    vec3  pa = p - a, ba = b - a;
...
map(in vec3 pos, float atime)
{
    ...
    if (deli - 1.0 < res.x) // bounding volume
    {
        ...
        {
            vec2 arms = sdStick(
              sq, vec3(0.18 - 0.06 * hr * sign(r.x), 0.2, -0.05), vec3(0.3 + 0.1 * p2, -0.2 + 0.3 * p2, -0.15), 0.03, 0.06);
...
vec4
raycast(in vec3 ro, in vec3 rd, float time)
{
...
float
calcSoftshadow(in vec3 ro, in vec3 rd, float time)
{
    float res  = 1.0;
    float tmax = 12.0;
...
vec3
calcNormal(in vec3 pos, float time)
{
...
float
calcOcclusion(in vec3 pos, in vec3 nor, float time)
{
    float occ = 0.0;
    float sca = 1.0;
    for (int i = 0; i < 5; i++) {
...
vec3
render(in vec3 ro, in vec3 rd, float time)
{
    // sky dome
...
vec4
HappyJumping(vec2 fragCoord, float time)
{
    vec3  tot  = vec3(0.0);
...
main()
{
    FragColorChannel0 = HappyJumping(vec2(1,1), 1);
}