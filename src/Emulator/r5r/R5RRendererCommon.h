...
template<typename T>
struct R5RRaytracerHitPoint
{
    Mesh<T>*       mesh;
    Vec3<T>        hitPoint;
    Vec3<T>        hitNormal;
    Vec3<T>        barycentricCoords;
    Vec3<uint32_t> hitTriangleIndices;
    T              distance;
};
...
template<typename T>
float
calculateReflectivity(Vec3<T> albedo, float metallic, float roughness)
{
...
template<typename T>
Vec3<T>
transformNormalToWorldSpace(const Vec3<T>& sampledNormal,
                            const Vec3<T>& tangent,
                            const Vec3<T>& biTangent,
                            const Vec3<T>& normal)
{
    Vec3<T> TT = tangent;
    TT.normalize();
