...
struct RasterizerEventListener
{
    RasterizerEventListener()
      : activated(false)
...
struct R5RRenderer
{
    explicit R5RRenderer();
    ~R5RRenderer();
    void render(Scene<float>* scene, RasterizerEventListener& listener);

#if defined(R5R_RASTERIZER_ENABLE)
    R5RRasterizer<float>* rasterizer;
#endif
#if defined(R5R_RAYTRACER_ENABLE) || defined(R5R_RASTERIZER_RAYTRACED_SHADOWS)
    R5RRaytracer<float>* raytracer;
#endif
};